...
As tres figuras do artigo (composicao setorial, distribuicao de participacoes por
organizacao e diversidade setorial por evento, ano de 2025) NAO sao produzidas por
este pipeline; sao geradas separadamente por gerar_figuras.py (secao 6), a partir
dos mesmos dados.
...
gerar_figuras.py
    Gera as tres figuras do artigo (fig_setor.png, fig_dist.png, fig_div.png,
    ano de 2025) a partir dos eventos-*.json. Re-extrai a rede de 2025 e
    auto-valida os agregados (n_nos, composicao, multiorg, multissetor) contra
    resultados_multiano.json antes de plotar.

estatisticas_tamanho_evento.py
    Estatisticas descritivas do tamanho dos eventos (numero de organizacoes por
    evento) por ano --- media, desvio, mediana e maximo. Grava
    estatisticas_tamanho_evento.json, fonte dos tamanhos medios reportados no
    artigo.
...
--------------------------------------------------------------------------------
    ...
    |- predicoes_classificador.csv          (saida de gerar_predicoes.py)
    |- kappa_resultado.json                 (saida de compute_kappa_final.py)
    |- estatisticas_tamanho_evento.json     (saida de estatisticas_tamanho_evento.py)
    |- resultados_multiano.json             (saida do pipeline de rede)
    |- resultados_multiano.xlsx             (saida do pipeline de rede)
    |- fig_setor.png / fig_dist.png / fig_div.png   (saidas de gerar_figuras.py)
    |- eventos-2023.json
    |- eventos-2024.json
    |- eventos-2025.json

Nota: por padrao o pipeline de rede grava em saida_multiano/ (variavel de ambiente
SAIDA_DIR); as copias versionadas de resultados_multiano.* estao na raiz.
